...
//Inici d'estructura per la prova de visió, que de moment l'únic que fa és esperar que s'apreti el botó esquerre o el dret i girar una quantitat de graus en aquella direcció. Aquest programa és la prova de que turn_angle() no és ni molt menys exacta.
...
void user_loop(void)
{
  ...
  switch(state)
  {
    ...
    case move: valor_actual = exp_compass_get_avg_heading();
    ...
    case move_l: if(exp_compass_get_avg_heading() > target_angle)
		  {
		    //turn_left();
		    turn_angle(-15);
		    state = move_l;
    ...
    case move_r: if(exp_compass_get_avg_heading() < target_angle)
		  {
		    //turn_right();
		    turn_angle(15);
		    state = move_r;
